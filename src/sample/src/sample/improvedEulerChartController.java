...
public class improvedEulerChartController implements Initializable, ChartControllerInterface {
    @FXML
    ...
    public void plotChart(){
        ...
        for(int i = 1; i <= N; i++) {
            ...
            double k2_exact = h * computeFunction(x[i-1] + h/2, computeExact(x[i-1]) + k1_exact);
            double localError = computeExact(x[i]) - computeExact(x[i-1] - k2_exact);
    ...
    public static double getMaxError(double new_n){
        ...
        for(int i = 1; i < new_n; i++) {
            ...
            double k2 = new_h * computeFunction(x + new_h/2, y + k1/2);
            y = y + k2;
            x = x + new_h;
